...
- 座標系: `x` は左→右で 0..100、`y` は下→上で 0..100。原点は左下、**x が東/右・y が北/上**。
- 用途: NavigationScreen が「1つ前の売り場 → 次の売り場」の**方位角（bearing）をローカル計算**するのに使う。

```mermaid
flowchart LR
  S["start (70,6)"] --> V1["地場野菜 (55,10)"] --> V["野菜 (86,55)"] --> F["果物 (86,30)"]
  F --> Fi["お魚 (86,88)"] --> M["お肉 (60,92)"] --> T["天ぷら (42,92)"]
  T --> Sd["惣菜 (28,86)"] --> D["乳製品 (32,64)"] --> Y["ヨーグルト (14,66)"]
  Y --> Fr["...冷凍/飲料/味噌/即席/豆腐..."] --> Ic["アイス・酒・菓子・台所・ゴミ袋"]
  Ic --> E["卵 (12,58)"] --> B["パン (12,48)"] --> Sc["セルフレジ (18,18)"] --> C["レジ (45,18)"]
```
